...
class MyFrame(wx.Frame):
  def __init__(self, parent, id, title):
		...
		label1= wx.StaticText(panel, -1, 'Speed (mps):')
		label2= wx.StaticText(panel, -1, 'Distance (km/m):')
		label3= wx.StaticText(panel, -1, 'Time (hrs/min):')
		...
		self.d=wx.ComboBox(self, -1, choices=['km','m'], style=wx.CB_DROPDOWN)
		self.time= wx.TextCtrl(panel, -1, '0')
		self.t=wx.ComboBox(self, -1, choices=['hr','min','sec'], style=wx.CB_DROPDOWN)
	...
	def onCalc(self, event):
		...
		if (self.speed.GetValue())=='0': # find speed
			d1=self.distance.GetValue()
			t1=self.time.GetValue()
			if(self.d.GetValue())=='km' and (self.t.GetValue())=='hr':				
				self.speed.SetValue(str(float(d1)*1000/(float(t1)*3600))) # set speed
			if(self.d.GetValue())=='km' and (self.t.GetValue())=='min':				
				self.speed.SetValue(str(float(d1)*1000/(float(t1)*60))) # set speed
			if(self.d.GetValue())=='km' and (self.t.GetValue())=='sec':				
				self.speed.SetValue(str(float(d1)*1000/(float(t1)))) # set speed
			if(self.d.GetValue())=='m' and (self.t.GetValue())=='hr':
				self.speed.SetValue(str(float(d1)/(float(t1)*3600))) # set speed
			if(self.d.GetValue())=='m' and (self.t.GetValue())=='min':
				self.speed.SetValue(str(float(d1)/(float(t1)*60))) # set speed
			elif(self.d.GetValue())=='m' and (self.t.GetValue())=='sec':
				self.speed.SetValue(str(float(d1)/(float(t1)))) # set speed
